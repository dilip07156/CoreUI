...
$(document).on('change', '.bindProductRange', function (e) {
    ...
    if (catId !== undefined && prodId !== undefined && catId !== '' && prodId !== '') {
        $.ajax({
            type: "GET",
            url: "/Cruise/GetProductRange",
            data: { ProductId: prodId, CategoryId: catId, AdditionalYN: "", positionname: "cruise" },
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (result) {
                var $elem = thisparent.find('select[name*="CruiseDetails[' + index + '].PositionRoomsData.SupplementID"]');
                var $elem2 = $('#CruiseDetails_' + index + '__PositionRoomsData_RoomDetails_0__ProductRangeID');
...
$('#tblCruise').on('click', 'a.icon-squre-green', function () {
    addRow(this);
    return false;
});
...
$('#tblCruise').on('click', 'a.icon-squre-red', function () {
    var clonedInputLen = $("#tblCruise .CruiseRow").length;
    if (clonedInputLen == 1) {
...
$("#tblCruise").on('click', '.clsfocprice', function () {
    ...
    if (curRow.find('.field-validation-error').length <= 0) {
        ...
        var model = { QRFId: QRFId, SaveType: 'partial', FOC: foc, Price: pr, MenuViewModel: MenuViewModel, CruiseDetails: cruiseDetails, __RequestVerificationToken: $('input[name="__RequestVerificationToken"]').val() };
...
/*----------------------------------cruise save button start here-----------------------------------------*/
$('.cruisesave').click(function () {
    var flag = true;
    if (!(RoomValidation("#frmCruise")))
        flag = false;
    ...
    if (flag) {
        ...
        $.ajax({
            ...
            dataType: "html",
            success: function (response) {
                if (response.indexOf('frmCruise') > 0) {
                    $('#frmCruise').replaceWith(response);
                    var form = $("#frmCruise");
                    ...
                    var successmsg = '<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a><strong>Error!</strong> Cruise not saved.</div>';
                    $('#frmCruise .simple-tab-cont').before(successmsg);
                }
            },
            error: function (response) {
...
/*----------------------------------cruise save button ends here-----------------------------------------*/

$(document).ready(function () {
    $("#tblCruise").on('change', '.keepAsChanged', function () {
        var row = $(this).closest('tr');
        if (this.value == 'Included') row.find('.NoOfPaxColumn').hide();
        else row.find('.NoOfPaxColumn').show();
    });

    $("#tblCruise").on('change', '.bindDayStartTime', function () {
        var row = $(this).closest('tr');
...
function addRow(thisparam) {
    var flag = true;
    $("#tblCruise .CruiseRow:not([style*='display: none;'])").each(function (index) {
    ...
    if (tdPrice.length > 0) {
        ...
        tdPrice.find('.tooltip-cont #tblCruise table tbody').empty();
    }

    $('#tblCruise tbody tr.CruiseRow').each(function (iRowCnt) {
        if (iRowCnt > index) {
            var thisRow = this;
            ...
            $(this).find('input,select,span,textarea,td,ul,div').each(function (iColCnt) {
                if (this.id != undefined) {
                    ...
                    // else if (this.id.indexOf('KeepAs') > -1) this.value = this.value;
                        ...
                        if (iRowCnt == (parseInt(index) + 1)) {
                            ...
                            else if (this.id.indexOf('ddlKeepAs') > -1) {
                                $(this).val("Included");
                            }
                            else if (this.id.indexOf('CruiseDetails') > -1) {
                                $(this).val("Through Coach");
                            }
...
function removeRow(thisparam, rowIndex) {
    ...
    $('#tblCruise .CruiseRow').each(function (iRowCnt) {
        if (iRowCnt >= rowIndex) {
            var curRow = $(this).find('#hdnSeqNo').val();
...
function currentRowValidation(currentRow) {
    ...
    if (ddlKeepAs == '') {
        ...
        if (currentRow.find("[id=CruiseDetails_" + index + "__NoOfPaxAdult]").val() == "0") {
            currentRow.find("[id=CruiseDetails_" + index + "__NoOfPaxAdult]").parents(".NoOfPaxColumn").find("span").text("*");
            IsValidFlag = false;
        }
    }
...
function AutoCompleteChanged(thisparam) {
    var parentrow = $(thisparam).parents('.CruiseRow');
    ...
    if (productid != "" && productid != null && productid != undefined) {
        $.ajax({
            type: "GET",
            url: "/Cruise/GetProductCategoryByParam",
