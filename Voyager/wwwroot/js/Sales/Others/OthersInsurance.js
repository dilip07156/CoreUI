$(document).ready(function () {
    var form = $("#frmOthersInsurance");
    form.removeData('validator');
    form.removeData('unobtrusiveValidation');
    $.validator.unobtrusive.parse(form);

    $("#OthersInsurance").on("keyup", "#tblInsurance .bindProductName", function () {
        //var index = $(this).closest('tr').find('#hdnSeqNo').val();
        var city = $(this).closest('tr').find('.bindCity').val();
        InitializeAutoComplete('/Activities/GetProductName', this, 3, { prodName: this.value, city: city, prodType: 'Insurance' }, 'category');
    });

    $('#OthersInsurance').off('click').on('click', '#tblInsurance a.icon-squre-green', function () {
        var flag = true;
        if (!(RoomValidation("#frmOthersInsurance")))
            flag = false;

        if (!$('#frmOthersInsurance').valid())
            flag = false;
    ...
    $('#OthersInsurance').on('click', '#tblInsurance a.icon-squre-red', function () {
        if (confirm("Are you sure you want to delete this row?")) {
            var index = $(this).closest('tr').find('#hdnSeqNo').val();
            var othersId = $('#OthersInsurance #LocalGuideDetails_' + index + '__OthersId').val();  //IsDeleted

            if (othersId != undefined && othersId != null && othersId != '' && othersId.length > 30 && othersId.substring(0, 8) != '00000000') {
                if ($('#tblInsurance tbody tr').length <= 1) {
                ...
                $('#OthersInsurance #LocalGuideDetails_' + index + '__IsDeleted').val('True');
            }
            else {
                removeRowIns(this, index);
            }
        }
        return false;
    });

    $("#OthersInsurance").on('change', '#tblInsurance .bindProductRange', function () {
        var index = $(this).closest('tr').find('#hdnSeqNo').val();
        var QRFId = $("#QRFId").val();
        var prodId = $('#OthersInsurance #ProductName2_' + index).val();
        var catId = $('#OthersInsurance #ProductCategoryID_' + index).val();
        if (catId !== undefined && prodId !== undefined && catId !== '' && prodId !== '') {
            $.ajax({
                ...
                success: function (result) {
                    $('#OthersInsurance #LocalGuideDetails_' + index + '__SupplierID').val(result.supplierId);
                    $('#OthersInsurance #LocalGuideDetails_' + index + '__SupplierName').val(result.supplierName);
                    if (result != undefined && result != null) {
                        $('#OthersInsurance #SupplementID_' + index).closest('tr').find('ul li.clonedInput').each(function (iCnt) {
                            if (iCnt > 0) {
                        ...
                        if (result.prodRangeList != undefined && result.prodRangeList != null) {
                            var $elem = $('#OthersInsurance #SupplementID_' + index);                                                                    //Supplementary Services
                            var $elem2 = $('#OthersInsurance #LocalGuideDetails_' + index + '__PositionRoomsData_RoomDetails_0__ProductRangeID');        //Mandatory Services
    ...
    $("#OthersInsurance").on('click', '#tblInsurance .clsfocprice', function () {
        var thisparam = this;
        var QRFId = $("#QRFId").val();
    ...
    $("#OthersInsurance").on('change', '#tblInsurance .bindDayStartTime', function () {
    ...
    $('.SaveInsurance').click(function () {
        ...
        if (flag) {
            var model = $('#frmOthersInsurance').serialize();
            ...
            $.ajax({
                ...
                dataType: "html",
                success: function (response) {
                    if (response.indexOf('frmOthersInsurance') > 0) {
                        $('#frmOthersInsurance').replaceWith(response);
                        var form = $("#frmOthersInsurance");
                        form.removeData('validator');
                        form.removeData('unobtrusiveValidation');
                        $.validator.unobtrusive.parse(form);
                        ...
                        $('#frmOthersInsurance .simple-tab-cont').before(successmsg);
                    }
                },
                error: function (response) {
                    alert(response.statusText);
...
function addRowIns(thisparam) {
    ...
    $('#OthersInsurance #tblInsurance tbody tr.InsuranceRow').each(function (iRowCnt) {
        if (iRowCnt > index) {
            ...
            $(this).find('input,select,span,textarea,td,ul').each(function (iColCnt) {
                if (this.id != undefined) {
                    ...
                    else if (this.id.indexOf('ddlDayID') > -1 || this.id.indexOf('KeepAs') > -1) this.value = this.value;
                    else {
                        if (iRowCnt == (parseInt(index) + 1)) {
                            if (this.id.indexOf('NoOfPax') > -1) { }
                            else if (this.id.indexOf('StartTime') > -1) {
                                var fullStartTime = $('#OthersInsurance #LocalGuideDetails_' + index + '__StartTime').val();
...
function removeRowIns(thisparam, rowIndex) {
    if ($('#OthersInsurance #tblInsurance tbody tr').length <= 1) {
        var clonedRow = $(thisparam).closest('tr').clone();
        $(thisparam).closest('tr').after(clonedRow);
        clonedRow.find('input,select,span,textarea').each(function (iColCnt) {
            if (this.value != undefined) {
    ...
    $('#OthersInsurance #tblInsurance tbody tr').each(function (iRowCnt) {
        if (iRowCnt >= rowIndex) {
            var curRow = $(this).find('#hdnSeqNo').val();
