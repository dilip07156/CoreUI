$(document).ready(function () {
    $("#OthersTabsCont").on("keyup", ".bindCity", function () {
        InitializeAutoComplete('/Quote/GetCityName', this, 3, { term: this.value });
    });

    $("#frmOthersLocalGuide #tblLocalGuide").off().on("keyup blur", ".bindProductName", function () {
        var index = $(this).closest('tr').find('#hdnSeqNo').val();
        var city = $(this).closest('tr').find('.bindCity').val();
        InitializeAutoComplete('/Activities/GetProductName', this, 3, { prodName: this.value, city: city, prodType: 'Guide,Assistant' }, 'category');
    });

    $('#frmOthersLocalGuide #tblLocalGuide').on('click', 'a.icon-squre-green', function () {
        var flag = true;
        if (!(RoomValidation("#frmOthersLocalGuide")))
            flag = false;

        if (!$('#frmOthersLocalGuide').valid())
            flag = false;
    ...
    $('#frmOthersLocalGuide #tblLocalGuide').on('click', 'a.icon-squre-red', function () {
        if (confirm("Are you sure you want to delete this row?")) {
            var index = $(this).closest('tr').find('#hdnSeqNo').val();
            var othersId = $('#LocalGuideDetails_' + index + '__OthersId').val();  //IsDeleted

            if (othersId != undefined && othersId != null && othersId != '' && othersId.length > 30 && othersId.substring(0, 8) != '00000000') {
                if ($('#tblLocalGuide tbody tr').length <= 1) {
                ...
                $('#LocalGuideDetails_' + index + '__IsDeleted').val('True');
            }
            else {
    ...
    $("#frmOthersLocalGuide #tblLocalGuide").on('change', '.bindProductRange', function () {
        var index = $(this).closest('tr').find('#hdnSeqNo').val();
        var QRFId = $("#QRFId").val();
        var prodId = $('#ProductName_' + index).val();
        var catId = $('#LocalGuideDetails_' + index + '__ProductCategoryID').val();
        if (catId !== undefined && prodId !== undefined && catId !== '' && prodId !== '') {
            $.ajax({
                ...
                success: function (result) {
                    $('#LocalGuideDetails_' + index + '__SupplierID').val(result.supplierId);
                    $('#LocalGuideDetails_' + index + '__SupplierName').val(result.supplierName);
                    if (result != undefined && result != null) {
                        ...
                        if (result.prodRangeList != undefined && result.prodRangeList != null) {
                            ...
                            var $elem2 = $('#LocalGuideDetails_' + index + '__PositionRoomsData_RoomDetails_0__ProductRangeID');        //Mandatory Services
    ...
    $("#frmOthersLocalGuide #tblLocalGuide").on('click', '.clsfocprice', function () {
        var thisparam = this;
        var QRFId = $("#QRFId").val();
        ...
        if (curRow.find('.field-validation-error').length <= 0) {
            ...
            LocalGuideDetails.push({
                OthersId: curRow.find("input[id*='__OthersId']").val(),
    ...
    $("#frmOthersLocalGuide #tblLocalGuide").on('change', '.bindDayStartTime', function () {
        var row = $(this).closest('tr');
        row.find('input[id*="__StartTime"]').val('10:00');
    });

    $('.SaveLocalGuide').click(function () {
        var flag = true;
        if (!(RoomValidation("#frmOthersLocalGuide")))
            flag = false;
        ...
        if (flag) {
            var model = $('#frmOthersLocalGuide').serialize();
            ...
            $.ajax({
                ...
                dataType: "html",
                success: function (response) {
                    if (response.indexOf('frmOthersLocalGuide') > 0) {
                        $('#frmOthersLocalGuide').replaceWith(response);
                        var form = $("#frmOthersLocalGuide");
                        ...
                        $('#frmOthersLocalGuide .simple-tab-cont').before(successmsg);
                    }
                },
                error: function (response) {
                    alert(response.statusText);
...
function addRow(thisparam) {
    ...
    $('#frmOthersLocalGuide #tblLocalGuide tbody tr.LocalGuideRow').each(function (iRowCnt) {
        if (iRowCnt > index) {
                    ...
                    if (this.id == 'hdnSeqNo') this.value = iRowCnt;
                        ...
                        if (iRowCnt == (parseInt(index) + 1)) {
                            if (this.id.indexOf('NoOfPax') > -1) { }
                                ...
                                var fullStartTime = $('#LocalGuideDetails_' + index + '__StartTime').val();
...
function removeRow(thisparam, rowIndex) {
    if ($('#frmOthersLocalGuide #tblLocalGuide tbody tr').length <= 1) {
        var clonedRow = $(thisparam).closest('tr').clone();
        $(thisparam).closest('tr').after(clonedRow);
        clonedRow.find('input,select,span,textarea').each(function (iColCnt) {
            if (this.value != undefined) {
    ...
    $('#frmOthersLocalGuide #tblLocalGuide tbody tr').each(function (iRowCnt) {
        if (iRowCnt >= rowIndex) {
            var curRow = $(this).find('#hdnSeqNo').val();
